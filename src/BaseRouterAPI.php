...
class BaseRouterAPI
{
    ...
    // public function connect($ip,$login,$password){
    //     $connect = $this->connect1($ip,$login,$password);
    //     if (count($connect) > 0) {
    //         if (isset($connect[0]) && $connect[0] != '!trap') {
    //             return true;
    //         }else{
    //             $connect = $this->connect2($ip,$login,$password);
    //             if (isset($connect[0]) && $connect[0] == '!done' && count($connect) == 1) {
    //                 return true;
    //             }else{
    //                 return false;
    //             }
    ...
    public function connect1($ip, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            $RESPONSE = [];
            $this->connected = false;
